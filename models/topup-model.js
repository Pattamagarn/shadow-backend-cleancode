    ...
    if(!isConnected){
        ...
        const getWallet = async() => {
            ...
            if(tw.status.code == 'SUCCESS'){
                ...
                connection.query('UPDATE finance SET cash_amount = cash_amount + ?, aysel_amount = aysel_amount + ?, update_at = ? WHERE email = ?', [baht, (process.env.AYSEL/process.env.BAHT)*baht, new Date(), requestEmail], (error, result) => {
                    if(error){
                        response.status(200).json({status: false, payload: 'เติม Aysel ล้มเหลว'})
                    }else{
                        connection.query('INSERT INTO history_payment (uuid, email, aysel_amount, cash_amount, payment_status, create_at, update_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [uuid.v4(), requestEmail, (process.env.AYSEL/process.env.BAHT)*baht, baht, true, new Date(), new Date()], (error, result) => {
                            if(error){
                                response.status(200).json({status: false, payload: 'เติม Aysel ล้มเหลว'})
                            }else{
